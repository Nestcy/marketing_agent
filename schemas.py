...
class CampaignPlan(BaseModel):
    ...
    def each_asset_declares_type(cls, assets: List[str]) -> List[str]:
        for asset in assets:
            if "image" not in asset.lower():
                raise ValueError(
                    f"Asset description {asset!r} must mention 'Image' "
                    f"so downstream routing can detect it — this platform "
                    f"generates images only, not video."
                )
        return assets
