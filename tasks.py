...
def _publish_due_assets_for_campaign(
    ...
    campaign_events.log_event(
        campaign_id,
        "cron_publish_run",
        payload={"published": list(due_images.keys()), "date": datetime.date.today().isoformat(), "status": status},
        source="cron",
    )
...
def request_reference_image(campaign_id: str, asset_id: str, reason: str) -> None:
    ...
    import campaign_events
    campaign_events.log_event(
        campaign_id, "reference_photo_requested", payload={"asset_id": asset_id, "reason": reason}, source="cron"
    )
